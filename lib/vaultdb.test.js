...
describe('VaultDb', function() {
	...
	describe('broadcast', () => {
		it('should set data to quickly expire', async () => {
			const db = new VaultDb();
			db.stop();
			db.$insert = jest.fn();
			...
			await db.broadcast('b', 2);
			expect(db.$insert).toBeCalledWith({ k: 'b', d: 2, t: now, e: now + db._debounce });
			expect(db.cleaner.setTimer).toBeCalledWith(now + db._debounce);
		});

		it('should not debounce the emitter', async () => {
			const db = new VaultDb();
			db.stop();
			db.$insert = jest.fn();
			db.cleaner.setTimer = jest.fn();
			...
			await db.broadcast('c', 3);
			expect(db.$insert).toBeCalledWith({ k: 'c', d: 3, t: now, e: now + db._debounce });
			expect(db.cleaner.setTimer).toBeCalledWith(now + db._debounce);
			expect(db.send).toBeCalledWith('c', false);
		...
		it('should accept an Object', async () => {
			...
			expect(db.$insert).toBeCalledWith({ k: 'a', d: 1, t: now, e: now + db._debounce });
			expect(db.$insert).toBeCalledWith({ k: 'b.c', d: 2, t: now, e: now + db._debounce });
			expect(db.$insert).toBeCalledTimes(2);
		});
	});
	...
	describe('set', () => {
		it('should set data', async () => {
			...
			expect(db.del).toBeCalledWith('a.b', now - 1, undefined, 1);
		...
		it('should set expiring data', async () => {
			...
			expect(db.del).toBeCalledWith('a.b', now - 1, expire - 1, 1);
			expect(db.emit).toBeCalledWith('$set');
			expect(db.send).toBeCalledWith('a.b');
	...
	describe('del', () => {
		...
		it('should accept an Array', async () => {
			...
			await db.del(['a', 'b'], 1, 2);
			expect(db.$remove).toBeCalledWith('a', 1, 2);
			expect(db.$remove).toBeCalledWith('b', 1, 2);
		});
