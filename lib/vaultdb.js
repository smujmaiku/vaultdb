...
class VaultDb {
	get EMIT_DELAY() { return 10; }
	get EXPIRE_DELAY() { return 100; }
	get UID_CHARS () { return '-0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ_abcdefghijklmnopqrstuvwxyz'; }
	...
	constructor(db) {
		...
		this._rootkeys = [];
		delay(VaultDb.EMIT_DELAY).then(() => {
			return this.$rootkeys();
		}).then((list) => {
			list.forEach((key) => this.trigger(key));
			return this.setClean(VaultDb.EMIT_DELAY);
		});
	}

	static newUid(now = Date.now(), chars = VaultDb.UID_CHARS) {
		return [].concat(
	...
	static uidToTime(uid = '', chars = VaultDb.UID_CHARS) {
	...
	emit(name, data) {
		...
		db[ekey] = setTimeout(_send.bind(db, data), VaultDb.EMIT_DELAY);
	...
	setClean(expire) {
		...
		db._expirei = setTimeout(db.clean.bind(db), Math.max(VaultDb.EXPIRE_DELAY, db._expiret - now));
	}
	...
	autobackup(path, wait, limit, debug) {
		...
		db._autobackup = () => {
			...
			return delay(Math.round(wait * 1e3)).then(() => {
				...
				return VaultDb.listBackups(path);
			}).then((list) => {
				list.slice(0, -limit)
					.forEach(bak => {
	...
	restoreBackup(path, id) {
		...
		return VaultDb.listBackups(path).then((list) => {
