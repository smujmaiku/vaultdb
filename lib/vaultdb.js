...
class VaultDb extends ObjDb.Emitter {
	...
	async broadcast(name, data) {
		if (common.getType(name) === 'object') {
			return Promise.all(common.eachKey(
				name,
				(k, d) => this.broadcast(k, d),
			));
		}

		const now = Date.now();
		const e = this._debounce + now;
	...
	async del(name, t = Date.now() + 1, e, skipEvents) {
		if (common.getType(name) === 'array') {
			return Promise.all(name.map(n => this.del(n, t, e, skipEvents)));
		}
